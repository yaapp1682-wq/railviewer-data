...
4. **直通の切り分け** — OSM のルート関係は運行系統（「○○線 普通 (A => B)」）
   であることが多く、乗り入れ先の線路・駅をそのまま取り込んでしまうため、
   way 名で自線の線路を切り分け。ただし切り分けたことで「駅まで線路が届かない」
   区間は、駅の並びに沿う範囲だけ戻す
5. **経路の生成** — 上下線が別 way の路線で経路が往復してしまうため、
   重複する way を除いて一方向の経路を作り直し。分岐は別の経路として出力
6. **クレンジング** — 建設中・廃線・構造物リレーションの除外、
   運休線（線路が disused タグ）の救済、誤所属駅
   （私鉄ブランド駅が JR 線に混入等）の自動除去。
   加えて全路線を人手で照合した除去リスト・補完リストを適用
7. **属性整備** — ふりがな、路線色（OSM colour タグ）、路線記号、事業者、
...
- 一部路線には線路ジオメトリの欠損による分断が残っています（廃止区間・
  第三セクター化により実際に線路が繋がっていない路線を含みます）
...
my鉄道マップ には、利用者が作った路線・ダイヤ・停車駅案内・路線図を
ファイルやリンクで他の人に渡せる機能があります（v1.0.6〜）。

渡されるファイル（`.myrail`）の中身と、ライセンスの扱いは次のとおりです。

| 中身 | 出どころ |
|---|---|
| 既存路線のダイヤ（駅名・座標・路線名を含む） | **このデータベース由来＝OpenStreetMap 由来** |
| 自作路線に取り込んだ既存駅（座標・名前） | **同上** |
| 自作の駅・路線、ダイヤの時刻、図の配置・色・題 | **利用者が作ったもの** |

- OpenStreetMap 由来の部分を含むファイルには、
  出典（`attribution`）・ライセンス（`license` = `ODbL-1.0`）・全文の場所（`licenseURL`）・
  元にした版（`source`）・説明文（`notice`）が**ファイル自体に記録されます**。
- 受け取った人のアプリにも同じ内容が表示され、**再配布するときも ODbL の条件が続きます**。
- **作った人が作成した部分**（ダイヤの時刻・図の配置・自作路線の形など）は ODbL の対象外で、
  その人の著作物です。共有した作品には
  [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/deed.ja) が記録されます
  （`workLicense` = `CC-BY-4.0`）。**自由に使えます。**手直しすることも、
  手直しした版を配ったり公開したりすることもできます（商用利用も可）。
  条件は、作品に記載されている名前（`credits`）を出典として示すことだけです。
  もらった作品に手を入れて渡した場合、`credits` には**最初に作った人から順に全員**が入ります。
  名前が1つも入っていなければ、出典の表示も求められません。
  共有画面の「送る前の確認」に同じ内容を表示しています。
- 自作の駅だけで作られた作品には ODbL を付けません。
  その人が一から作ったものにまで継承を求めることになるためです。
- リンク共有は作品を公開のサーバーに1件置いて URL を渡す仕組みです。
  リンクを受け取った人はデータそのものを取得できます（ODbL の共有条件を満たします）。
  リンクは作成から30日で切れます。

ライセンス全文は [LICENSE](LICENSE) にあります。
